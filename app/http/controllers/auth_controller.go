...
func (c *AuthController) AddUser(ctx *gin.Context) {
    var req struct {
    ...
    err := ctx.BindJSON(&req)
    if err != nil {
        c.ResponseJson(ctx, http.StatusNotFound, "404", nil)
        return
    }
    if req.Secret != config.LoadConfig().Secret {
        c.ResponseJson(ctx, http.StatusNotFound, "404", nil)
        return
    }

    u, err := user.CreateUser(req.Name, req.Password)
    c.ResponseJson(ctx, http.StatusOK, "", gin.H{
        "info": u,
    })
}
