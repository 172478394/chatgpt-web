...
func init() {
    var err error
    chatClient, err = chat.NewClient("chat", client.WithHostPorts(config.LoadConfig().ApiURL), client.WithConnectTimeout(5*time.Second))
    if err != nil {
        panic(err)
    }
}
...
func (c *ChatController) CompletionService(ctx *gin.Context) {
    ...
    if msgLen > 5 {
        request.Messages = request.Messages[msgLen-5:]
    }
    //for i, msg := range request.Messages {
    //    msgRune := []rune(msg.Content)
    //    if msg.Role == "assistant" && len(msgRune) > 500 {
